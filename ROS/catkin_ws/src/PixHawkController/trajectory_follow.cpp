...
int previous_vel = -1000;       //any arbitaryily large negative value
...
static const int home = (m_size+1)/2;          //coordinate of robot or car
...
void set_velocity()
{
  ...
  if(navigation_status!=2)
  {
    ...
    double x1=home,y1=home,x2,y2,x3,y3; // To read the waypoints from traj_file           //default x1,y1 = 51;
    ...
    while(getline (traj_file, waypoint_xy))                         //(traj_file >> x2) && (traj_file >> y2)
    {
      ...
      else if(  (stream >> x3) && (stream >> y3)  )
      {
        ...
        {
          if(sqrt(pow(x3/*/scale_factor*/ - home/*/scale_factor*/,2) + pow(y3/*/scale_factor*/ - home/*/scale_factor*/,2))>10)    //if(sqrt(pow(x3/scale_factor - 51/scale_factor,2) + pow(y3/scale_factor - 51/scale_factor,2))>10)
            continue;
          // double m1=0,m2=0;
    ...
    else if (status.OmplSolStatus == -1)
    {
      desired_velocity = 3;
    }
    else
...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber dist_sub = n.subscribe<std_msgs::Float64>("/dist_obst", 1, dist_callback, noDelay);
  ...
  while(ros::ok())
  {
    ...
    des_vel_msg.velocity = (float)desired_velocity;
    //cout<<"vel -- "<<des_vel_msg.velocity<<endl;
    des_vel_msg.priority = 1;
    des_vel_msg.node_id = 11;
